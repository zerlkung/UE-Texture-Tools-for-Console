...
class Utexture:
    ...
    def __serialize_uexp(self, ar: ArchiveBase, ubulk_start_offset: int = 0, uptnl_start_offset: int = 0):
        ...
        if ar.is_reading:
            # Consume remaining uexp bytes (PS5/FModel exports may have padding)
            remaining = ar.size - ar.tell()
            if remaining > 4:
                self._uexp_padding = ar.read(remaining - 4)
                self.uexp_size = ar.tell() - start_offset
            ...
            # Write back padding if any
            if self._uexp_padding:
                ar.write(self._uexp_padding)
                self.uexp_size = ar.tell() - start_offset
            # Preserve original skip_offset for PS5 format
            # (skip_offset is only recalculated if it's a standard UE format)
            has_ps5_mips = any(getattr(m, '_ps5_fields', {}).get('type') in (1, 2) for m in self.mipmaps)
            if not has_ps5_mips:
                current = ar.tell()
    ...
    def get_dds(self) -> DDS:
        ...
        # Filter to only mips with actual data
        valid_mips = [m for m in self.mipmaps if getattr(m, 'has_data', True) and m.width > 0 and m.height > 0]
        if not valid_mips:
            valid_mips = [self.mipmaps[0]]  # fallback to first mip

        # make dds header
        header = DDSHeader()
        w, h = self.get_max_size()
        header.update(
            w, h, self.get_depth(), len(valid_mips),
